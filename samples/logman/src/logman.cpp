...
void wmain(void)
{
    ...
    ENABLE_TRACE_PARAMETERS* pTraceParameters;
    pTraceParameters->EnableProperty;
    pTraceParameters->

    // Create the trace session.

    status = StartTrace((PTRACEHANDLE)&SessionHandle, LOGSESSION_NAME, pSessionProperties); //append the session name
...
//EnableTrace/EnableTraceEx enable/disable providers update level/keywords/extended data/filter data(each time you
// call the EnableTrace or EableTraceEx funtion, ETW calls the provider's callback function

//stop session: ControlTrace(EVENT_TRACE_CONTROL_STOP )

//manifest-based provider: filter(keyword,level)
classic provider: EnableTrace manifest-based provider: EnableTraceEx(with filters, event payload, scope, stack walk)
TraceSetInformation: trace additional information on an event
*/