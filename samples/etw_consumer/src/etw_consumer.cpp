...
#define LOGFILE_PATH "E:\\test\\etw_test\\DxgKrnl\\Merged.etl"
...
void HandleDxgKrnlEvent(PEVENT_RECORD pRecord);
...
void wmain(void)
{
    ...
    trace.LogFileName = (LPSTR)LOGFILE_PATH;
...
DWORD FormatAndPrintData(PEVENT_RECORD pEvent, USHORT InType, USHORT OutType, PBYTE pData, DWORD DataSize, PEVENT_MAP_INFO pMapInfo)
{
    ...
    case TDH_INTYPE_BINARY:
    {
        if (TDH_OUTTYPE_IPV6 == OutType)
        {
            ...
            fnRtlIpv6AddressToString = (PIPV6ADDRTOSTRING)GetProcAddress(
                GetModuleHandle(LPCSTR("ntdll")), LPCSTR("RtlIpv6AddressToStringW"));
            ...
            fnRtlIpv6AddressToString((IN6_ADDR*)pData, LPTSTR(IPv6AddressAsString));
    ...
    case TDH_INTYPE_SID:
    {
        ...
        if (!LookupAccountSid(NULL, (PSID)pData, LPSTR(UserName), &cchUserSize, LPSTR(DomainName), &cchDomainSize, &eNameUse))
        {
...
bool ReadTDHPropertyByName(const std::string& propertyName, PEVENT_RECORD eventRecord,
    PROPERTY_TYPE& propertyValue)
{
    PROPERTY_DATA_DESCRIPTOR descriptor;

    std::wstring wstrPropertyName(propertyName.begin(), propertyName.end());

    descriptor.PropertyName = reinterpret_cast<ULONGLONG>(wstrPropertyName.c_str());
    descriptor.ArrayIndex = ULONG_MAX;

    auto result = TdhGetProperty(eventRecord, 0,
        NULL, 1, &descriptor, sizeof(PROPERTY_TYPE), reinterpret_cast<PBYTE>(&propertyValue));

    if (ERROR_SUCCESS != result)
    {
        std::ostringstream errorMessage;
        errorMessage << "The TdhGetProperty call failed during trying find the "
            << propertyName << " property. Error: " << result;
        return false;
    }
...
void HandleDxgKrnlEvent(PEVENT_RECORD pRecord) {
    uint64_t pDxgAdapter = 0;
    uint64_t nodeOrdinal = 0;
    uint64_t engineType = 0;
    std::vector<uint8_t> friendlyNameBuffer;

    ReadTDHPropertyByName("pDxgAdapter", pRecord, pDxgAdapter);
    ReadTDHPropertyByName("NodeOrdinal", pRecord, nodeOrdinal);
    ReadTDHPropertyByName("EngineType", pRecord, engineType);

    std::cout << "-----------------------dxgadaptor: " << pDxgAdapter << "\tnode: " << nodeOrdinal << "\tengine: " << engineType << std::endl;
}